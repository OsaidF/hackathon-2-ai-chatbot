...
- Idempotency test (completing already completed task)
"""
...
class TestCompleteTask:
    ...
    def test_complete_task_success_case(self):
        """Test successful task completion - should FAIL until implementation"""
        # Arrange
        complete_data = {
        ...
        with patch.object(self.server, '_mark_task_completed') as mock_complete:
            ...
            with pytest.raises(NotImplementedError):
                result = self.server.complete_task(**complete_data)
    ...
    def test_complete_task_idempotency(self):
        """Test completing already completed task - should succeed (idempotent)"""
        complete_data = {
            "user_id": self.valid_user_id,
            "task_id": self.valid_task_id
        }
        ...
        with pytest.raises(NotImplementedError):
            # Mock that task is already completed
            with patch.object(self.server, '_get_task_by_id') as mock_get_task:
                ...
                with patch.object(self.server, '_mark_task_completed') as mock_complete:
                    mock_complete.return_value = {
                        ...
                        "title": "Buy groceries",
                        "completed": True,
                        "updated_at": "2025-01-24T11:00:00Z"  # Same timestamp for idempotent operation
                    }

                    result = self.server.complete_task(**complete_data)

                    # Should return completed status without error
                    assert result["status"] == "completed"
                    assert result["completed"] is True
        ...
        with pytest.raises(NotImplementedError):
            for user_id in invalid_user_ids:
                ...
                with pytest.raises(Exception) as exc_info:
                    self.server.complete_task(**complete_data)
    ...
    def test_complete_task_missing_user_id(self):
    ...
    def test_complete_task_different_user(self):
    ...
    def test_complete_task_database_error(self):
        ...
        with pytest.raises(NotImplementedError):
            with patch.object(self.server, '_mark_task_completed') as mock_complete:
                mock_complete.side_effect = Exception("Database connection failed")
    ...
    def test_complete_task_output_format_contract_compliance(self):
    ...
    def test_complete_task_updated_timestamp_updated(self):
        """Test that updated_at timestamp is modified on completion"""
        complete_data = {
            "user_id": self.valid_user_id,
            "task_id": self.valid_task_id
        }
            ...
            with patch.object(self.server, '_get_task_by_id') as mock_get_task:
                ...
                with patch.object(self.server, '_mark_task_completed') as mock_complete:
                    completed_task = {
                        ...
                        "title": "Buy groceries",
                        "completed": True,
                        "updated_at": "2025-01-24T11:00:00Z"  # Different timestamp
                    }
